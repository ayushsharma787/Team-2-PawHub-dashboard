...
PL = dict(paper_bgcolor="rgba(0,0,0,0)",
          plot_bgcolor="rgba(255,248,240,0.4)",
          font=dict(family="Plus Jakarta Sans,Inter,sans-serif",
                    color="#3E2A1A", size=11),
...
html,body,[class*="css"]{font-family:'Plus Jakarta Sans',sans-serif;background:#FFF8F0}
...
div[data-testid="metric-container"]{
  background:#fff!important;border:1px solid #E8D5C4!important;
  border-left:4px solid #6F4E37!important;border-radius:14px!important;
  padding:16px 18px!important;
  box-shadow:0 4px 16px rgba(111,78,55,.1)!important}
div[data-testid="metric-container"] label{
  color:#9E8272!important;font-size:0.72rem!important;font-weight:700!important;
  text-transform:uppercase!important;letter-spacing:.1em!important}
div[data-testid="metric-container"] div[data-testid="stMetricValue"]{
  color:#3E2A1A!important;font-size:1.6rem!important;font-weight:800!important}
...
.icard{background:#fff;border:1px solid #E8D5C4;border-radius:14px;
       padding:20px;margin-bottom:14px;box-shadow:0 4px 16px rgba(111,78,55,.07)}
.shead{color:#6F4E37;font-size:1.1rem;font-weight:700;border-bottom:2px solid #D4860B;
       padding-bottom:6px;margin:22px 0 14px}
.ibox{background:linear-gradient(135deg,rgba(212,168,83,.08),rgba(255,248,240,.6));
      border-left:4px solid #D4860B;border-radius:0 10px 10px 0;
      padding:12px 16px;margin:10px 0 18px;color:#3E2A1A;font-size:.88rem}
.lbar{display:flex;align-items:center;justify-content:space-between;
      padding:9px 18px;border-radius:12px;margin-bottom:16px;
      background:linear-gradient(135deg,rgba(26,18,9,.9),rgba(13,10,6,.95));
...
def ring(pct, color, label, size=88, stroke=8):
    ...
    return (f'<div style="text-align:center">'
            ...
            f'<text x="{size//2}" y="{size//2+13}" text-anchor="middle" fill="#9E8272" '
            f'font-size="8" font-weight="600">{label}</text>'
            f'</svg></div>')

def pc(fig, h=320, **kw):
    """Apply PL base layout plus any extra kwargs — avoids duplicate key errors."""
    layout = {**PL, "height": h, **kw}
    fig.update_layout(**layout)
    st.plotly_chart(fig, use_container_width=True, config=PCFG)

def pd_chart(fig, h=360, **kw):
    layout = {**PD, "height": h, **kw}
    fig.update_layout(**layout)
    st.plotly_chart(fig, use_container_width=True, config=PCFG)
...
# ══════════════════════════════════════════════════════════════════════════════
if page == "🏠  Home":
    st.markdown("<h1 style='color:#6F4E37;font-size:1.8rem;margin-bottom:4px'>Welcome to PawIndia</h1>"
                "<p style='color:#9E8272'>India's first super-app for dog owners — validated by real market research</p>",
                unsafe_allow_html=True)
    live_bar(n)

    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("Respondents",     f"{n:,}")
    c2.metric("Will Download",   f"{pct_yes}%",         delta="Definite Yes")
    c3.metric("Open to Trying",  f"{pct_maybe}%",       delta="Conditional")
    c4.metric("Avg Spend/Month", f"Rs.{avg_spend:,}")
    c5.metric("Total Reachable", f"{pct_yes+pct_maybe}%", delta="Yes + Maybe")
    ...
    with cl:
        ...
        pc(fig, h=320, margin=dict(t=10,b=10,l=0,r=40),
    ...
    with cr:
        ...
        pc(fig2, h=320, margin=dict(t=10,b=10,l=0,r=40),
           xaxis=dict(showgrid=False,visible=False), yaxis=dict(tickfont=dict(size=10)))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 style='color:#6F4E37;font-size:1.8rem;margin-bottom:4px'>Know Your City</h1>"
                "<p style='color:#9E8272'>Find dog-friendly vets, parks, cafes and grooming near you</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    if filtered.empty:
        ...
        shead(f"3D Map: {city} — Column height shows rating")
        try:
            import pydeck as pdk
            filtered["color"] = filtered["category"].apply(
                lambda c: CAT_COLORS.get(c,[111,78,55]) + [200])
            filtered["elevation"] = (filtered["rating"]-3.5)*2000
            lat0, lon0 = CITY_COORDS[city]
            col_layer = pdk.Layer("ColumnLayer", data=filtered, get_position=["lon","lat"],
                                   get_elevation="elevation", elevation_scale=1, radius=120,
                                   get_fill_color="color", pickable=True, auto_highlight=True)
            scat_layer = pdk.Layer("ScatterplotLayer", data=filtered, get_position=["lon","lat"],
                                    get_color="color", get_radius=80, pickable=True)
            view = pdk.ViewState(latitude=lat0, longitude=lon0, zoom=12, pitch=55, bearing=15)
            tooltip = {"html": "<div style='background:rgba(26,18,9,.95);border:1px solid rgba(212,168,83,.3);"
                               "border-radius:10px;padding:12px;font-family:sans-serif'>"
                               "<b style='color:#D4A853'>{name}</b><br>"
                               "<span style='color:#C4B99A'>{category}</span><br>"
                               "<span style='color:#EDE4D3'>{rating} ★ | {reviews} reviews</span><br>"
                               "<span style='color:#9E8272'>{note}</span></div>",
                       "style": {"background":"transparent","border":"none"}}
            st.pydeck_chart(pdk.Deck(layers=[col_layer, scat_layer],
                                      initial_view_state=view, tooltip=tooltip,
                                      map_style="mapbox://styles/mapbox/dark-v10"))
            st.caption("Taller columns = higher rated. Hover for details. Drag to rotate, scroll to zoom.")
        except Exception:
            lat0, lon0 = CITY_COORDS[city]
            fig_map = go.Figure(go.Scattermapbox(
                lat=filtered["lat"], lon=filtered["lon"],
                mode="markers",
                marker=dict(size=14, color=[f"rgb({CAT_COLORS.get(c,[100,100,100])[0]},"
                                             f"{CAT_COLORS.get(c,[100,100,100])[1]},"
                                             f"{CAT_COLORS.get(c,[100,100,100])[2]})"
                                             for c in filtered["category"]]),
                text=filtered.apply(lambda r: f"{r['name']} | {r['rating']}★", axis=1),
                hovertemplate="%{text}<extra></extra>",
            ))
            fig_map.update_layout(
                mapbox=dict(style="carto-positron", center=dict(lat=lat0, lon=lon0), zoom=11),
                height=460, margin=dict(t=0,b=0,l=0,r=0), paper_bgcolor="rgba(0,0,0,0)")
        ...
        for i, (_, row) in enumerate(filtered.iterrows()):
            ...
            cols3[i%3].markdown(
                ...
                f'<div style="color:#9E8272;font-size:.8rem">{row["reviews"]} reviews</div>'
        ...
        if sat_data:
            ...
            fig_s.add_hline(y=3, line_dash="dash", line_color="#9E8272", annotation_text="Neutral (3.0)")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 style='color:#6F4E37;font-size:1.8rem;margin-bottom:4px'>Dog Owner Insights</h1>"
                "<p style='color:#9E8272'>Who are India's dog owners, what do they spend, and what problems do they have?</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    with t2:
        ...
        fig3.update_layout(height=300, margin=dict(t=10,b=0),
                           paper_bgcolor="rgba(0,0,0,0)", showlegend=False, font=dict(color="#3E2A1A"))
    ...
    with t3:
        ...
        with c2:
            ...
            pc(fig3, h=260, margin=dict(t=10,b=20), yaxis=dict(showgrid=False))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 style='color:#6F4E37;font-size:1.8rem;margin-bottom:4px'>Will They Buy?</h1>"
                "<p style='color:#9E8272'>Subscription preferences, who is most likely to adopt, and what would make them pay</p>",
                unsafe_allow_html=True)
    live_bar(n)

    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Definite Yes",   f"{pct_yes}%",   delta="Would download now")
    ...
    with t1:
        ...
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,248,240,.3)",
                          height=360, margin=dict(t=20,b=20,l=20,r=20), font=dict(color="#3E2A1A"))
        st.plotly_chart(fig, use_container_width=True, config=PCFG)
    ...
    with t2:
        ...
        with c2:
            ...
            pc(fig2, h=300, margin=dict(t=10,b=20),
               xaxis=dict(showgrid=False,visible=False),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<h1 style='color:#6F4E37;font-size:1.8rem;margin-bottom:4px'>Data Journey</h1>"
                "<p style='color:#9E8272'>How we built 2,000 realistic survey responses and cleaned them for analysis</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    with t3:
        ...
        fig4 = go.Figure(go.Bar(x=tc.index, y=tc.values,
                                 ...
                                 textposition="outside"))
        pc(fig4, h=260, margin=dict(t=10,b=20), yaxis=dict(showgrid=False))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Dark header
    st.markdown(
        f'<div style="background:linear-gradient(135deg,#1A1209,#0D0A06);border-radius:14px;'
        f'padding:22px 26px;margin-bottom:14px;border:1px solid rgba(212,168,83,.15)">'
        ...
        f'font-weight:800;color:#EDE4D3">Research & Analytics</h1>'
        f'<div style="color:#A07D3A;font-size:.88rem;margin-top:4px">'
        f'Full ML analysis: Classification, Clustering, Association Rules, Regression</div>'
        f'</div></div></div>', unsafe_allow_html=True)

    st.markdown(
        f'<div style="background:rgba(26,18,9,.7);border:1px solid rgba(212,168,83,.15);'
        f'border-radius:10px;padding:10px 16px;margin-bottom:16px;color:#C4B99A;font-size:.88rem">'
        f'&#9679; All models are cached after first load for fast page switching &nbsp;|&nbsp; '
    ...
    def dhead(t):
        st.markdown(f'<div style="display:flex;align-items:center;gap:10px;margin:22px 0 12px">'
                    f'<div style="width:4px;height:18px;background:linear-gradient(180deg,{HONEY},transparent);'
                    f'border-radius:2px"></div>'
                    f'<div style="color:#EDE4D3;font-size:1.05rem;font-weight:700">{t}</div></div>',
                    unsafe_allow_html=True)

    def dibox(t):
        st.markdown(f'<div style="background:rgba(212,168,83,.07);border-left:4px solid {HONEY};'
                    f'border-radius:0 8px 8px 0;padding:12px 16px;margin:8px 0 16px;'
                    f'color:#C4B99A;font-size:.87rem">&#128161; {t}</div>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        st.markdown(f"<p style='color:#7A6F5C;font-size:.88rem'>8-9 algorithms compared. "
        ...
        if best in feat_imp:
            ...
            pd_chart(fig_fi, h=400, margin=dict(t=10,b=10), xaxis=dict(showgrid=False,visible=False))
    ...
    with t2:
        ...
        st.markdown(f"<p style='color:#7A6F5C;font-size:.88rem'>K-Means on 10 behavioural and demographic features. "
        ...
        for i, (cid, (name, color, desc)) in enumerate(personas.items()):
            ...
            cols4[i].markdown(
                ...
                f'<div style="color:#7A6F5C;font-size:.75rem;margin-bottom:8px">{desc}</div>'
    ...
    with t3:
        ...
        st.markdown(f"<p style='color:#7A6F5C;font-size:.88rem'>Apriori algorithm on Q10 (problems) "
        ...
        if rules_ok:
            ...
            filtered_r = rules[(rules["support"]>=min_sup) &
                                ...
                                (rules["lift"]>=min_lift)]
            st.markdown(f"<p style='color:#7A6F5C;font-size:.84rem'>{len(filtered_r)} rules found.</p>",
                        unsafe_allow_html=True)
    ...
    with t4:
        ...
        st.markdown(f"<p style='color:#7A6F5C;font-size:.88rem'>Target: Q7 monthly spend (INR). "
