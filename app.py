...
# ── Text colours used everywhere ─────────────────────────────────────────────
INK    = "#2C1810"   # darkest — headings
DARK   = "#4A2C1A"   # dark — body text
MID    = "#5C3D2E"   # medium — captions, notes

# ── Plotly base layouts ───────────────────────────────────────────────────────
# IMPORTANT: tickfont and title_font are set explicitly here.
# _merge_layout does a 3-level deep merge so callers that pass xaxis/yaxis
# kwargs will EXTEND these rather than overwrite them.
_AXIS_LIGHT = dict(
    gridcolor="rgba(111,78,55,.08)",
    linecolor="rgba(111,78,55,.15)",
    zeroline=False,
    tickfont=dict(color=INK, size=11, family="Plus Jakarta Sans,Inter,sans-serif"),
    title_font=dict(color=DARK, size=11, family="Plus Jakarta Sans,Inter,sans-serif"),
)
_AXIS_DARK = dict(
    gridcolor="rgba(80,65,40,.15)",
    linecolor="rgba(80,65,40,.3)",
    zeroline=False,
    tickfont=dict(color="#EDE4D3", size=11, family="Plus Jakarta Sans,Inter,sans-serif"),
    title_font=dict(color="#EDE4D3", size=11, family="Plus Jakarta Sans,Inter,sans-serif"),
)

PL = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(255,252,248,0.7)",
    font=dict(family="Plus Jakarta Sans,Inter,sans-serif", color=INK, size=11),
    xaxis=dict(**_AXIS_LIGHT),
    yaxis=dict(**_AXIS_LIGHT),
...
PD = dict(
    ...
    font=dict(family="Plus Jakarta Sans,Inter,sans-serif", color="#EDE4D3", size=11),
    xaxis=dict(**_AXIS_DARK),
    yaxis=dict(**_AXIS_DARK),
...
<div style="text-align:center;margin-top:6px">
  ...
  <div style="color:#7A6F5C;font-size:8px;letter-spacing:1.5px;text-transform:uppercase;margin-top:2px">
...
html,body,[class*="css"]{font-family:'Plus Jakarta Sans',sans-serif;background:#FFFCF8;color:#2C1810}
.stApp{background:#FFFCF8!important}
.main .block-container{background:#FFFCF8!important}
.block-container{padding-top:1.2rem;padding-bottom:2rem;max-width:1400px}
/* Force all Streamlit text to dark brown */
.stMarkdown p, .stMarkdown li, .stMarkdown span,
[data-testid="stText"], label, .stSelectbox label,
.stSlider label, .stRadio label, [data-baseweb="select"] span,
[data-testid="stWidgetLabel"] p {color:#2C1810!important;font-weight:500}
/* Selectbox and slider specific fix */
[data-testid="stSelectbox"] label p {color:#2C1810!important;font-weight:600!important}
[data-testid="stSlider"] label p {color:#2C1810!important;font-weight:600!important}
[data-baseweb="select"] [data-testid="stMarkdownContainer"] p {color:#2C1810!important}
/* Sidebar */
...
.stTabs [data-baseweb="tab"]{
  ...
  color:#4A2C1A!important;font-size:12.5px!important;font-weight:600!important;
...
[data-testid="stExpander"]{
  border:1px solid #E0CFC0!important;border-radius:12px!important;
  background:#FFFFFF!important;margin-top:20px!important}
[data-testid="stExpander"] summary p{
  color:#4A2C1A!important;font-weight:700!important;font-size:.9rem!important}
/* Cards and helpers */
...
.ibox{background:linear-gradient(135deg,rgba(212,168,83,.1),rgba(255,252,248,.8));
      ...
      padding:12px 16px;margin:10px 0 18px;color:#2C1810;font-size:.88rem;font-weight:500}
...
@keyframes floatCard{
  0%,100%{transform:translateY(0);box-shadow:0 4px 20px rgba(111,78,55,.12)}
  50%{transform:translateY(-4px);box-shadow:0 8px 28px rgba(111,78,55,.18)}}
</style>
...
CAT_HEX    = {"Vet":"#E53935",   "Park":"#4CAF50",   "Cafe":"#D4860B",    "Grooming":"#6F4E37"}
...
def tab_summary(points: list[str]):
...
def live_bar(n):
    ...
    st.markdown(
        ...
        f'<span style="color:#D4A853;font-size:11px;font-weight:600">{n:,} respondents loaded</span></div>'
...
def ring(pct, color, label, size=88, stroke=8):
    ...
    return (
        ...
        f'<text x="{size//2}" y="{size//2+13}" text-anchor="middle" fill="{MID}" '
...
def pc(fig, h=320, **kw):
    """Cream chart — deep merge so axis tickfonts survive."""
    fig.update_layout(**_deep_merge(PL, {"height": h, **kw}))
    st.plotly_chart(fig, width="stretch", config=PCFG)

def pd_chart(fig, h=360, **kw):
    """Dark chart — deep merge so axis tickfonts survive."""
    fig.update_layout(**_deep_merge(PD, {"height": h, **kw}))
    st.plotly_chart(fig, width="stretch", config=PCFG)
...
def _kpi(label, value, sub, color, delay="0s"):
    st.markdown(
        ...
        f'<div style="color:#7A5C44;font-size:.68rem;font-weight:700;'
        ...
        f'<div style="color:{MID};font-size:.75rem;margin-top:5px;font-weight:500">{sub}</div>'
        f'</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("<p style='font-size:.65rem;color:#5a5040;text-align:center;line-height:1.8'>"
                "PawIndia © 2024<br><span style='color:#D4A853'>Market Research Dashboard</span></p>",
                unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
if page == "🏠  Home":
    st.markdown(f"<h1 style='color:{INK};font-size:1.8rem;margin-bottom:4px'>Welcome to PawIndia</h1>"
                f"<p style='color:{DARK};font-weight:500'>India's first super-app for dog owners — validated by real market research</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    with rc[4]:
        fig = go.Figure(go.Pie(
            ...
            textinfo="percent", textfont=dict(size=11, color=INK),
        ))
    ...
    st.markdown(
        ...
        f'<div style="color:{INK};font-size:.95rem;line-height:1.7">'
        ...
        f'The survey says <strong style="color:#2E7D32">{pct_yes+pct_maybe:.1f}% are open to it</strong> '
    ...
    with cl:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker=dict(color=ch_df["Respondents"], colorscale=[[0,"#F0E4D7"],[0.5,HONEY],[1,BROWN]]),
        ...
        pc(fig, h=340,
           ...
           yaxis=dict(title="", tickfont=dict(color=INK, size=10)))
    ...
    ibox(f"Yes, Maybe and No groups have similar median spend (around Rs.{yes_s:,}/month). "
         f"Willingness to download is driven more by digital habits and pain points than by how much "
         f"someone already spends — which means PawIndia can win across all spending levels.")
    ...
    tab_summary([
        f"{pct_yes:.1f}% of surveyed dog owners would definitely download PawIndia, "
        f"and a further {pct_maybe:.1f}% are open to it — giving a total reachable market of {pct_yes+pct_maybe:.1f}%.",
        ...
        f"The average Indian dog owner spends {avg_spend_k} per month on their pet, "
        "suggesting real willingness to pay for services that solve genuine problems.",
    ])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h1 style='color:{INK};font-size:1.8rem;margin-bottom:4px'>Know Your City</h1>"
                f"<p style='color:{DARK};font-weight:500'>Find dog-friendly vets, parks, cafes and grooming near you</p>",
                unsafe_allow_html=True)
    live_bar(n)

    st.markdown('<div class="ibox">📌 <strong>Note:</strong> Places shown are simulated for '
                'demonstration. At launch PawIndia will connect to live Google Places data.</div>',
                unsafe_allow_html=True)

    # Filters — styled labels via CSS override above
    ...
    # ── Street-level OpenStreetMap for selected city ──────────────────────────
    shead(f"Street Map — {city}")
    lat0, lon0 = CITY_COORDS[city]
    fig_map = go.Figure()

    # Grey context pins (filtered out places)
    if len(grey_out) > 0:
        fig_map.add_trace(go.Scattermap(
            ...
            text=grey_out.apply(
                lambda r: f"<b style='color:#555'>{r['name']}</b><br>"
                          f"{r['category']} | {r['rating']} ★<br>"
                          f"<i>(below filter)</i>", axis=1),
            hovertemplate="%{text}<extra></extra>",
            name="Other places (filtered out)",
            showlegend=True,
        ))

    # Coloured matching pins
    for cat_name, hex_col in CAT_HEX.items():
        ...
        fig_map.add_trace(go.Scattermap(
            ...
            name=f"{CAT_ICONS[cat_name]} {cat_name}",
        ))
    ...
    st.plotly_chart(fig_map, width="stretch", config=PCFG)
    ibox(f"Showing {len(filtered)} matching place(s) in {city}. "
         f"Greyed pins are places that exist but don't match your current filter. "
         f"Larger pins mean higher rated. Hover any pin for details.")
    ...
    fig_india.add_trace(go.Scattergeo(
        ...
        mode="markers+text",
        marker=dict(size=other["total"]*6, color="rgba(111,78,55,0.25)",
                    line=dict(color=BROWN, width=1.5)),
        text=other["city"], textposition="top center",
        textfont=dict(size=11, color=BROWN),
    ...
    fig_india.add_trace(go.Scattergeo(
        ...
        mode="markers+text",
        marker=dict(size=sel["total"]*9, color=HONEY, line=dict(color=BROWN, width=2.5)),
        text=[city], textposition="top center",
        textfont=dict(size=13, color=BROWN, family="Bricolage Grotesque"),
        hovertemplate=f"<b>{city}</b><br>Vets: {sel['vets']} | Parks: {sel['parks']}<extra></extra>",
    ...
    # Place cards
    if len(filtered) > 0:
        shead(f"{CAT_ICONS.get(cat,'🐾')} Places in {city} matching your filters ({len(filtered)} found)")
        cols3 = st.columns(3)
        for i, (_, row) in enumerate(filtered.iterrows()):
            r, g, b = CAT_COLORS.get(row["category"], [111,78,55])
            cols3[i % 3].markdown(
                f'<div style="background:#fff;border:1px solid #E0CFC0;'
                f'border-top:3px solid rgb({r},{g},{b});border-radius:12px;'
                f'padding:16px;margin-bottom:12px;box-shadow:0 2px 12px rgba(111,78,55,.07)">'
                f'<div style="font-size:1.2rem">{CAT_ICONS.get(row["category"],"")}</div>'
                f'<div style="font-weight:700;color:{INK};margin:4px 0">{row["name"]}</div>'
                f'<div style="color:{AMBER};font-weight:600">{"★"*int(row["rating"])} {row["rating"]}</div>'
                f'<div style="color:{MID};font-size:.8rem">{row["reviews"]} reviews</div>'
                f'<div style="color:{BROWN};font-size:.8rem;margin-top:4px;font-style:italic">{row["note"]}</div>'
                f'</div>', unsafe_allow_html=True)
    ...
    if sat_data:
        ...
        fig_s.add_hline(y=3, line_dash="dash", line_color=MID,
    ...
    tab_summary([
        f"Every city tier scores below neutral (3.0/5) for satisfaction with pet-friendly spaces — "
        ...
        f"{len(city_df)} places are mapped in {city}. At launch, PawIndia will use live Google Places "
        "data to show thousands more.",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h1 style='color:{INK};font-size:1.8rem;margin-bottom:4px'>Dog Owner Insights</h1>"
                f"<p style='color:{DARK};font-weight:500'>Who are India's dog owners, what do they spend, and what problems do they have?</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    with t1:
        ...
        with c1:
            ...
            pc(fig, h=280,
               margin=dict(t=10, b=20, l=60, r=20),
               title=dict(text="Respondents by Age Group", font=dict(color=INK)),
        ...
        with c2:
            ...
            fig2 = go.Figure(go.Pie(
                labels=cc2.index, values=cc2.values,
                marker_colors=CHART, hole=0.45,
        ...
        fig_hm = go.Figure(go.Heatmap(
            ...
            colorbar=dict(tickfont=dict(color=INK), title=dict(text="Count", font=dict(color=INK))),
        ))
        pc(fig_hm, h=300,
        ...
        with c3:
            ...
            pc(fig3, h=260,
               ...
               title=dict(text="Dogs Owned per Household", font=dict(color=INK)),
        ...
        with c4:
            ...
            pc(fig4, h=260,
               ...
               title=dict(text="Dog Breeds / Types", font=dict(color=INK)),
    ...
    with t2:
        ...
        with c1:
            ...
            pc(fig, h=280,
               margin=dict(t=10, b=20, l=60, r=20),
               title=dict(text="Distribution of Monthly Spend", font=dict(color=INK)),
        ...
        with c2:
            ...
            fig2 = go.Figure(go.Bar(
                ...
                marker_color=[BROWN, AMBER, HONEY, "#C49A6C"],
                text=[f"Rs.{v:,}" for v in cs_df2["Avg Monthly Spend"]],
                textposition="outside",
                textfont=dict(color=INK, size=11),
            ))
            pc(fig2, h=280,
               margin=dict(t=10, b=20, l=70, r=20),
               title=dict(text="Avg Spend by City Tier", font=dict(color=INK)),
               xaxis=dict(title="City tier"),
        ...
        fig3 = go.Figure(go.Pie(
            labels=cat_c.index, values=cat_c.values,
            marker_colors=CHART, hole=0.4,
            textinfo="label+percent",
            textfont=dict(size=11, color=INK),
        ))
    ...
    with t4:
        ...
        with c1:
            ...
            pc(fig, h=260,
               margin=dict(t=40, b=20, l=60, r=20),
               title=dict(text="Current App Usage for Pet Needs", font=dict(color=INK)),
        ...
        with c2:
            ...
            fig2 = go.Figure(go.Pie(
                labels=pc2.index, values=pc2.values,
                marker_colors=CHART, hole=0.4,
            ...
            fig2.update_layout(height=260, margin=dict(t=40,b=0),
                               paper_bgcolor="rgba(0,0,0,0)", showlegend=False,
                               font=dict(color=INK),
                               title=dict(text="Where They Buy Pet Products", font=dict(color=INK,size=13)))
            st.plotly_chart(fig2, width="stretch", config=PCFG)
        ...
        fig4 = go.Figure(go.Heatmap(
            ...
            showscale=True,
            colorbar=dict(tickfont=dict(color=INK), title=dict(text="Correlation", font=dict(color=INK))),
        ))
        pc(fig4, h=360,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h1 style='color:{INK};font-size:1.8rem;margin-bottom:4px'>Will They Buy?</h1>"
                f"<p style='color:{DARK};font-weight:500'>Subscription preferences, who is most likely to adopt, and what would make them pay</p>",
                unsafe_allow_html=True)
    live_bar(n)

    c1, c2, c3, c4 = st.columns(4)
    with c1: _kpi("Definite Yes",    f"{pct_yes:.1f}%",           "Would download now",    "#2E7D32")
    with c2: _kpi("Open to It",      f"{pct_maybe:.1f}%",         "Conditional adopters",  AMBER)
    with c3: _kpi("Would Not",       f"{pct_no:.1f}%",            "Lost audience",         "#C62828")
    with c4: _kpi("Total Reachable", f"{pct_yes+pct_maybe:.1f}%", "Yes + Maybe",           TEAL)
    ...
    with t1:
        ...
        fig = go.Figure(go.Funnel(
            ...
            marker=dict(color=[BROWN, AMBER, HONEY, "#4CAF50"],
                        line=dict(color="#FFFCF8", width=1.5)),
            textfont=dict(color=INK, size=13),
        ))
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,252,248,.3)",
                          height=360, margin=dict(t=20,b=20,l=20,r=20),
                          font=dict(color=INK))
        st.plotly_chart(fig, width="stretch", config=PCFG)
        ibox(f"The funnel narrows from {total:,} surveyed to {definite:,} definite downloaders ({pct_yes:.1f}%). "
             f"The gap between 'open' ({willing:,}) and 'definite' ({definite:,}) is the conversion challenge — "
             "closing it requires building trust through a polished onboarding and a compelling first session.")
        ...
        tab_summary([
            f"{pct_yes:.1f}% would definitely download — a strong validation signal for a new product.",
            f"A further {pct_maybe:.1f}% are open to it — the conversion gap is real but closeable with good onboarding.",
    ...
    with t2:
        ...
        with c1:
            ...
            fig = go.Figure(go.Pie(
                labels=sub_c.index, values=sub_c.values,
                marker_colors=CHART, hole=0.45,
                textinfo="label+percent",
                textfont=dict(size=11, color=INK),
            ))
        ...
        with c2:
            ...
            fig2 = go.Figure(go.Bar(
                ...
                marker=dict(color=sa_df["% Definite Yes"],
                            colorscale=[[0,"#F0E4D7"],[0.5,HONEY],[1,BROWN]]),
                text=[f"{v}%" for v in sa_df["% Definite Yes"]], textposition="outside",
                textfont=dict(color=INK, size=11),
            ))
            pc(fig2, h=300,
               margin=dict(t=40, b=20, l=180, r=60),
               title=dict(text="Download Intent by Subscription Preference", font=dict(color=INK)),
        ...
        st.plotly_chart(fig3, width="stretch", config=PCFG)
        ibox(f"{pct_share}% of respondents will share their location with the app — "
             "enough to make the nearby vet, park and groomer features fully functional from launch.")
        ...
        tab_summary([
            ...
            f"{pct_share}% will share location, validating the core 'find nearby' features.",
        ])
    ...
    with t4:
        ...
        with c1:
            ...
            pc(fig, h=260,
               margin=dict(t=40, b=20, l=60, r=20),
               title=dict(text="% Definite Yes by Age Group", font=dict(color=INK)),
               xaxis=dict(title="Age group"),
        ...
        with c2:
            ...
            pc(fig2, h=260,
               margin=dict(t=40, b=20, l=60, r=20),
               title=dict(text="% Definite Yes by Number of Dogs Owned", font=dict(color=INK)),
               xaxis=dict(title="Number of dogs"),
               yaxis=dict(title="% who say Yes", showgrid=False, range=[0,80], rangemode="tozero"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"<h1 style='color:{INK};font-size:1.8rem;margin-bottom:4px'>Data Journey</h1>"
                f"<p style='color:{DARK};font-weight:500'>How we built 2,000 realistic survey responses and cleaned them for analysis</p>",
                unsafe_allow_html=True)
    live_bar(n)
    ...
    with t1:
        ...
        st.markdown(f'<div class="icard"><p style="color:{INK};line-height:1.8">'
        ...
        for i, (title, desc) in enumerate([
            ...
            st.markdown(
                f'<div style="background:#fff;border:1px solid #E0CFC0;border-top:4px solid {BROWN};'
                ...
                f'<strong style="color:{INK}">{title}</strong>'
                f'<p style="color:{DARK};margin:6px 0 0 34px;font-size:.88rem">{desc}</p>'
                f'</div>', unsafe_allow_html=True)
        ...
        for title, desc in [
            ...
            st.markdown(
                ...
                f'padding:10px 14px;background:#FFFCF8;border-radius:10px;border:1px solid #E0CFC0">'
                f'<span style="color:{AMBER};font-size:1rem;margin-right:10px;flex-shrink:0">⚠</span>'
                f'<div><strong style="color:{INK}">{title}</strong>'
                f'<p style="color:{DARK};margin:2px 0;font-size:.86rem">{desc}</p>'
                f'</div></div>', unsafe_allow_html=True)
    ...
    with t2:
        ...
        for step, title, desc in [
            ...
            st.markdown(
                ...
                f'<strong style="color:{INK};display:block;margin:4px 0">{title}</strong>'
                f'<p style="color:{DARK};margin:0;font-size:.88rem">{desc}</p></div>',
                unsafe_allow_html=True)
    ...
    with t3:
        ...
        ibox(f"The raw dataset had {raw_miss:,} missing values across all columns. "
             "After cleaning, the count is exactly zero — every row is complete and usable.")
        ...
        with c1:
            ...
            pc(fig2, h=240,
               margin=dict(t=40, b=20, l=60, r=20),
               title=dict(text="Raw — includes errors", font=dict(color=INK)),
               xaxis=dict(title="Monthly spend (INR)"),
        ...
        with c2:
            ...
            pc(fig3, h=240,
               margin=dict(t=40, b=20, l=60, r=20),
               title=dict(text="Cleaned — outliers removed", font=dict(color=INK)),
               xaxis=dict(title="Monthly spend (INR)"),
               yaxis=dict(title="Count", showgrid=False))
        ...
        tab_summary([
            f"Cleaning removed {len(raw)-n} rows and eliminated {raw_miss:,} missing values.",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'font-weight:800;color:{INK}">Research & Analytics</h1>'
        f'<div style="color:{DARK};font-size:.88rem;margin-top:4px">'
        f'Full ML analysis: Classification · Clustering · Association Rules · Regression'
        f'</div></div></div></div>', unsafe_allow_html=True)

    st.markdown(
        f'<div style="background:#FFF8F0;border:1px solid #E0CFC0;border-radius:10px;'
        f'padding:10px 16px;margin-bottom:16px;color:{INK};font-size:.88rem;font-weight:500">'
        f'● Models are cached after first load — switching tabs is instant &nbsp;|&nbsp; '
    ...
    def dhead(t):
        st.markdown(
            ...
            f'<div style="color:{INK};font-size:1.05rem;font-weight:700">{t}</div></div>',
            unsafe_allow_html=True)

    def dibox(t):
        st.markdown(
            f'<div class="ibox">💡 {t}</div>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        st.markdown(f"<p style='color:{DARK}'>8–9 algorithms compared on accuracy, precision, recall, F1-score "
                    "and 5-fold cross-validation. 80/20 stratified train/test split.</p>", unsafe_allow_html=True)
        ...
        fig_tbl = go.Figure(go.Table(
            header=dict(values=[f"<b>{c}</b>" for c in rdf.columns],
                        fill_color=BROWN, font=dict(color="#FFF8F0",size=12), align="left",
                        line=dict(color="#E0CFC0",width=1)),
            cells=dict(values=[rdf[c].astype(str).tolist() for c in rdf.columns],
                       fill_color=["#FFFCF8"]*len(rdf.columns),
                       font=dict(color=INK, size=11), align="left",
                       line=dict(color="#E0CFC0",width=1), height=30)))
        fig_tbl.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                               height=min(60+len(rdf)*34, 460),
                               margin=dict(l=0,r=0,t=0,b=0),
                               font=dict(color=INK))
        st.plotly_chart(fig_tbl, width="stretch", config=PCFG)
        dibox(f"Best model: {best} — F1={rdf.iloc[0]['F1-Score']:.3f}, "
              f"Accuracy={rdf.iloc[0]['Accuracy']:.3f}. "
              "Cross-validation confirms the result holds on data the model never saw during training.")
        ...
        if best in feat_imp:
            dhead(f"What Drives Predictions — Feature Importance ({best})")
            fi = feat_imp[best]
            ...
            fig_fi = go.Figure(go.Bar(
                ...
                marker=dict(color=fi_df["Importance"],colorscale=[[0,"#F0E4D7"],[0.5,HONEY],[1,BROWN]]),
        ...
        tab_summary([
            f"The best classifier ({best}) achieves F1={rdf.iloc[0]['F1-Score']:.3f} on held-out test data.",
    ...
    with t2:
        ...
        st.markdown(f"<p style='color:{DARK}'>K-Means on 10 behavioural and demographic features. "
        ...
        with c1:
            ...
            pc(fig_el, h=260,
               ...
               title=dict(text="Elbow Curve", font=dict(color=INK)),
        ...
        with c2:
            ...
            pc(fig_sil, h=260,
               ...
               title=dict(text="Silhouette Score by K", font=dict(color=INK)),
               xaxis=dict(title="Number of clusters (K)"),
        ...
        for i, (cid, (name, color, desc)) in enumerate(personas.items()):
            ...
            cols4[i].markdown(
                ...
                f'box-shadow:0 2px 12px rgba(111,78,55,.07)">'
                f'<div style="font-size:1.8rem">{emojis[i]}</div>'
                f'<div style="font-weight:700;color:{INK};font-size:.9rem;margin:6px 0">{name}</div>'
                f'<div style="color:{MID};font-size:.75rem;margin-bottom:8px">{desc}</div>'
                f'<div style="color:{color};font-weight:700;font-size:1rem">{int(row["Count"])} members</div>'
                f'<div style="color:{DARK};font-size:.78rem;margin-top:4px">'
        ...
        fig_tbl2 = go.Figure(go.Table(
            ...
            cells=dict(values=[disp[c].tolist() for c in disp.columns],
                       fill_color="#FFFCF8", font=dict(color=INK,size=11),
                       align="left", height=30)))
        fig_tbl2.update_layout(paper_bgcolor="rgba(0,0,0,0)", height=200,
                                margin=dict(l=0,r=0,t=0,b=0), font=dict(color=INK))
        ...
        if valid_pc:
            ...
            fig_pc = go.Figure(go.Parcoords(
                line=dict(color=pcd["cluster"],
                          ...
                          showscale=True,
                          colorbar=dict(tickfont=dict(color=INK))),
                dimensions=[dict(range=[pcd[c].min(),pcd[c].max()],
                                  label=lab, values=pcd[c],
                                  tickfont=dict(color=INK),
                                  labelside="bottom")
                             for c, lab in zip(valid_pc, pc_labels)],
                labelfont=dict(color=INK, size=11),
    ...
    with t3:
        ...
        st.markdown(f"<p style='color:{DARK}'>Apriori algorithm. Min support: 8%, Min lift: 1.1. "
        ...
        try:
            ...
            rules_ok = False
            st.error(f"mlxtend required. Install with: pip install mlxtend. Error: {e}")
        ...
        if rules_ok:
            ...
            st.markdown(f"<p style='color:{DARK}'>{len(filtered_r)} rules match current filters.</p>",
                        unsafe_allow_html=True)
            ...
            fig_rt = go.Figure(go.Table(
                ...
                cells=dict(values=[disp_r[c].tolist() for c in disp_r.columns],
                           fill_color="#FFFCF8", font=dict(color=INK,size=11),
                           align="left", height=30)))
            fig_rt.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                                  height=min(60+len(disp_r)*32, 520),
                                  margin=dict(l=0,r=0,t=0,b=0),
                                  font=dict(color=INK))
            ...
            if len(filtered_r) > 0:
                ...
                fig_bub = px.scatter(
                    ...
                    color_continuous_scale=[[0,HONEY],[1,BROWN]],
                    size_max=30,
    ...
    with t4:
        ...
        st.markdown(f"<p style='color:{DARK}'>Target: Q7 monthly spend (INR). "
                    "Linear, Ridge (L2) and Lasso (L1) regression compared on R², RMSE and MAE.</p>",
                    unsafe_allow_html=True)
        ...
        fig_rt2 = go.Figure(go.Table(
            header=dict(values=[f"<b>{c}</b>" for c in reg_res.columns],
                        fill_color=BROWN, font=dict(color="#FFF8F0",size=12), align="left"),
            cells=dict(values=[reg_res[c].tolist() for c in reg_res.columns],
                       fill_color="#FFFCF8", font=dict(color=INK,size=11),
                       align="left", height=30)))
        fig_rt2.update_layout(paper_bgcolor="rgba(0,0,0,0)", height=170,
                               margin=dict(l=0,r=0,t=0,b=0), font=dict(color=INK))
        st.plotly_chart(fig_rt2, width="stretch", config=PCFG)
        best_r = reg_res.sort_values("R²",ascending=False).iloc[0]["Model"]
        dibox(f"Best model: {best_r}. Ridge regularisation handles correlated one-hot features better "
              "than plain linear regression. Lasso automatically zeroes out the weakest predictors, "
              "confirming only a small set of variables really drives spending.")

        dhead("Actual vs Predicted Spend — All Three Models")
        cols_r = st.columns(3)
        for col_w, (name, (actual, predicted)) in zip(cols_r, preds.items()):
            with col_w:
                idx = np.random.RandomState(42).choice(len(actual), min(250,len(actual)), replace=False)
                ...
                pc(fig_av, h=260,
                   ...
                   title=dict(text=f"{name} — R²={r2v}", font=dict(color=INK)),
