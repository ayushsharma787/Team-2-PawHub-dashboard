# ml_engine.py — All ML logic cached for fast dashboard load
...
def run_classification():
    ...
    for name, model in models.items():
        ...
        cv   = cross_val_score(model, Xtr_fit, ytr, cv=5, scoring="accuracy").mean()
...
def run_clustering():
    ...
    _, df = load_data()
    cluster_cols = [
        "Q1_age_group_enc","Q7_monthly_spend_inr","Q9_online_purchase_freq_enc",
        ...
        "Q17_location_sharing_Yes Always","Q18_subscription_pref_Freemium",
    ...
    persona_map = {
        ...
        2: ("Casual Companion",   "#9B7CB6", "Any tier, low spend, passive engagement"),
    ...
    summary["Color"]   = summary["cluster"].map(lambda x: persona_map[x][1])

    return list(range(2,9)), inertias, sil_scores, labels, df2, summary, persona_map
...
def run_association_rules():
    ...
    _, df = load_data()
    ch_cols = [c for c in df.columns if c.startswith("Q10_challenges__")]
    ft_cols = [c for c in df.columns if c.startswith("Q14_preferred_features__")]
    basket  = df[ch_cols + ft_cols].fillna(0).astype(bool)
...
def run_regression():
    ...
    X, y3, y2, feat_names = build_feature_matrix()
    _, df = load_data()
    y_reg = df["Q7_monthly_spend_inr"].fillna(df["Q7_monthly_spend_inr"].median()).iloc[:len(X)]
